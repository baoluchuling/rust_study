    ...
    pub fn invoke(&mut self, path: String) -> String {
        ...
        if self.map.contains(_method) {
            if _method == "/" {
                ...
                _contents =
                    fs::read_to_string(ROOT_PATH.to_string() + &"/assets/hello.html").unwrap();
                ...
                _status_line = "HTTP/1.1 200 OK\r\n\r\n";
                _contents = ("{\"code\": 200, \"data\": ".to_string()
                    + &(deserialized.a + deserialized.b).to_string()
                    + ", \"message\": \"ok\"}")
                    .to_string();
            }
        } else {
